    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          {(activeLesson.contentType === 'VIDEO' || activeLesson.contentType === 'VIDEO_PDF') && (
            <div className="space-y-3">
              <div className="flex items-center justify-between text-xs font-bold text-slate-700 dark:text-slate-200">
                ...
                <span className="text-slate-500 dark:text-slate-400 font-mono text-[11px]">Anti-Skip Enforced</span>
              </div>
              <AntiSkipVideoPlayer
        ...
        <div className="lg:col-span-4 space-y-4">
          <Card>
            <CardHeader className="flex-row items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
              <CardTitle className="text-sm">Induction Navigation</CardTitle>
              <span className="text-xs font-bold text-blue-600 dark:text-blue-400 font-mono">{overallProgressPercentage}% Complete</span>
            </CardHeader>
                  ...
                  <div key={mod.id} className="space-y-2">
                    <div
                      className={`flex items-center justify-between p-2.5 rounded-xl text-xs font-bold ${
                        mod.isUnlocked
                          ? isDept
                            ? 'bg-purple-50 dark:bg-purple-950/50 text-purple-900 dark:text-purple-300 border border-purple-200 dark:border-purple-800/80'
                            : 'bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-100'
                          : 'bg-slate-50 dark:bg-slate-900/40 text-slate-400 dark:text-slate-500 border border-slate-200/60 dark:border-slate-800'
                      }`}
                    >
                          ...
                          <button
                            ...
                            className={`w-full text-left p-3 rounded-xl text-xs transition-all flex items-center justify-between ${
                              isActive
                                ...
                                : les.isUnlocked
                                ? 'bg-white dark:bg-slate-800/80 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-200 border border-slate-200/60 dark:border-slate-700/60'
                                : 'bg-slate-50/50 dark:bg-slate-900/30 text-slate-400 dark:text-slate-600 cursor-not-allowed border border-transparent'
                            }`}
                          >
