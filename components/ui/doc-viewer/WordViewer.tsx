...
export function WordViewer({ fileUrl, title }: WordViewerProps) {
  const [loading, setLoading] = useState(true);
  const [hasError, setHasError] = useState(false);
  const [userFallback, setUserFallback] = useState(false);

  const embedUrl = getEmbedUrl(fileUrl, 'word');
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      if (loading) {
        console.warn('Word viewer load timeout.');
      }
    }, 6000);
  ...
  if (hasError || userFallback) {
    return (
      <DocumentFallback
        ...
        title={title}
        category="word"
        categoryLabel="Word Document (DOCX)"
        onRetry={() => {
          setHasError(false);
          setUserFallback(false);
          setLoading(true);
    ...
    <div className="w-full h-[650px] bg-slate-900 relative overflow-hidden flex flex-col">
      ...
      <div className="px-4 py-2 bg-slate-950/90 border-t border-slate-800 text-slate-400 text-xs flex items-center justify-between z-20">
        <span className="flex items-center gap-1.5 text-[11px] font-medium text-slate-400">
          ...
          <span>Word Web Viewer Active</span>
        </span>
        ...
        >
          <span>Having trouble loading document? Switch to Direct View</span>
