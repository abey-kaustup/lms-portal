...
export function PDFViewerComponent({ fileUrl, title }: PDFViewerComponentProps) {
  const [loading, setLoading] = useState(true);
  const [hasError, setHasError] = useState(false);
  const [userFallback, setUserFallback] = useState(false);

  const embedUrl = getEmbedUrl(fileUrl, 'pdf');
  ...
  return (
    <div className="w-full h-[650px] bg-slate-950 relative overflow-hidden flex flex-col">
      {loading && (
      ...
      <div className="px-4 py-2 bg-slate-950 border-t border-slate-800 text-slate-400 text-xs flex items-center justify-between z-20">
        <span className="flex items-center gap-1.5 text-[11px] font-medium text-slate-400">
          <FileText className="w-3.5 h-3.5 text-rose-400" />
          <span>Native PDF Viewer Active</span>
        </span>
        ...
        >
          <span>Trouble loading PDF? Switch to Direct View</span>
